...
If running on windows, run the following first:
```bash
./gradlew wrapper
```
```bash
cd ..
docker build -t t3cicdbackend-configservice:latest -f configservice/Dockerfile .
docker build -t t3cicdbackend-pipelineservice:latest -f pipelineservice/Dockerfile .
```
...
## Update Backend Cluster after Code Change:
1. Rebuild image
```bash
cd ..
docker build -t t3cicdbackend-configservice:latest -f configservice/Dockerfile .
...
2. Remove old images
```bash
minikube ssh -- "docker rmi t3cicdbackend-configservice:latest --force"
minikube ssh -- "docker rmi t3cicdbackend-pipelineservice:latest --force"
```
...
minikube image load t3cicdbackend-pipelineservice:latest
```
3. Force k8s to use updated image:
```bash
kubectl rollout restart deployment configservice -n t3cicdbackend
kubectl rollout restart deployment pipelineservice -n t3cicdbackend
```
4. Verify
```bash
kubectl get pods -n t3cicdbackend
# Check if AGE is refreshed
```